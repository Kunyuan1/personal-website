...
| 2026-09-10 | [The fleet departs](./2026-09-10-the-fleet-departs.md) | Trisolaris can finally be lost, to unbound escape rather than a collision — measured at zero occurrences over 150 simulated minutes, since the scorching always arrives first. Gated at civilisation 50, and the ending persists as a fact rather than a state, so it is never what a returning visitor lands on. Harness now 42/42, plus nine review findings fixed — including a Begin again that restarted nothing, because the frame loop still held the system it was replacing |
| 2026-09-10 | [Hibernation](./2026-09-10-hibernation.md) | A returning visitor is told what they missed. The rate the ticket was built on had gone 61% stale sitting still — 102 civilisations an hour against the 63.2 it recorded — so the constant now ships with a row that re-measures it. Harness now 43/43. Six review findings fixed, including a reduced-motion visitor whose absence was measured from the start of their previous session |
| 2026-09-10 | [Red Coast](./2026-09-10-red-coast.md) | The contact page becomes a transmission, with a live console reading the same simulation as the hero — and `/contact` still prerenders. Also repaired `check:glyphs`, whose unused-glyph warning could never fire because the declaration counts as a use; it then found 20 orphaned glyphs, and the subset went 97 -> 77. Nine review findings fixed, including a console that froze at Chaotic forever once the fleet departed, and the repaired check being made to actually exit non-zero |
| 2026-09-10 | [The sophon on the 404](./2026-09-10-sophon-404.md) | The 404 becomes a sophon unfolding — a locked door rather than a weapon, which is what a 404 is. The droplet it replaced, and its nine review findings, move to #10 where the cursor will consume them; `canvas-ground.ts` came out of that review and is what makes this one warm with the page. The glyph check caught three separate drifts in two days. Ten review findings fixed, including a sphere that punched a dark hole in a red page because the canvas was transparent all along |
| 2026-09-11 | [Dimensional reduction](./2026-09-11-dimensional-reduction.md) | The whole page arrives as one flattened plane and opens into three dimensions on a first visit — nav, name, headline and the simulation together. Built first inside the canvas, which flattened the simulation while the page around it stood upright: a picture of a flattened thing rather than the idea, and reverted. It is CSS decided by a blocking script in `<head>`, because a statically prerendered page decided after hydration shows itself and *then* collapses. The finished animation leaves no transform at all — a `forwards` fill holds `scaleY(1)`, and an identity transform is still a containing block that would have followed a visitor onto the 404. The easing was measured, not chosen: the obvious ease-out was half open at 10% of the duration and 97% at half of it, and would have read as a snap. Six review findings fixed, two critical — the reduced-motion hero never drew at all in production, because a patch script had silently dropped the draw call *and* assigning `canvas.width` clears the canvas with nothing to repaint it (0 lit pixels before, 71 after); and a tab hidden at load spent the one-shot against animation frames that never arrived. A range-cut also quietly deleted the heat-publishing block, which `tsc` and `eslint` between them reported as three warnings |
