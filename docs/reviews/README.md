...
| 2026-09-11 | [The arrival](./2026-09-11-the-arrival.md) | A first visit opens on a black screen with a photon on it and one line from the thing holding it, then the screen tears off the page in a glitch and leaves as a horizontal band. Two earlier versions were built and thrown away, and the second is the instructive one: scaling the real document broke everything it touched - the canvas sized its buffer from a rect that reports post-transform geometry, measured 1px against a layout height of 712, and drew the whole simulation into one pixel with no resize event to recover it. An overlay deforms nothing, so the simulation is already running behind the tear. The halo took three attempts: a box-shadow that banded into a visible rectangle, a gradient that read as a disc with an edge, and one faint enough to vanish before its own boundary. One review finding survives from the previous round and predates the branch - the reduced-motion hero never drew in production, invisible to every check because React's double-mount hides it in development (0 lit pixels before, 71 after). A range-cut also quietly deleted the heat-publishing block, which `tsc` and `eslint` between them reported as three warnings |
