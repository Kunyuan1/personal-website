...
| 2026-09-10 | [A droplet alone in the void](./2026-09-10-droplet-404.md) | The 404 gets a droplet, and `src/lib/droplet.ts` is written here rather than inherited from #10 — there was no renderer to reuse. `滴` had to be restored to the subset, which the newly-enforcing glyph check caught as a red build rather than a silent font fallback. The provider's cost on a page with no canvas is measured for the first time: 0.09 ms a frame, 0.54% of one core. Nine review findings fixed, including highlights that provably never moved because the rotation cancelled itself, and a droplet that stayed cold on a page that goes red |
