...
| 2026-09-11 | [Dimensional reduction](./2026-09-11-dimensional-reduction.md) | The hero starts flat on a first visit and opens into three dimensions. The flat state was not luminous and had to be made so — peak 100 → 174 — and the no-white-out promise is a measurement planted against a deliberately blown version, 0.005% against 0.464%, which also proved peak luminance cannot see the difference at all. Three ways the one-shot could have been silently destroyed, all found by driving the page: a deep link burning it, a departure outranked by React's double-mount, and a guard reading microtask state that was still null. Six review findings fixed on top, two critical: the reduced-motion hero never drew at all in production — a patch script had silently dropped the draw call, and restoring it was still not enough, because assigning `canvas.width` clears the canvas and nothing repainted it — and a tab hidden at load spent the one-shot against animation frames that never arrived. The horizon also turned out to be compositing `source-over`, occluding 78% of the system it claimed to be the resting place of |
